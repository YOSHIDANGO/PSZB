...
const reelMap = [
 ['replay','bell','cherry','bar','blue7','hero','suika','bell','replay','red7','cherry','bell','bar','suika','blue7','replay','bell','cherry','red7','hero','replay'],
 ['cherry','bell','suika','replay','blue7','bar','hero','bell','cherry','red7','suika','replay','bell','blue7','cherry','bar','replay','suika','bell','hero','red7'],
 ['bar','replay','blue7','suika','hero','cherry','bell','red7','replay','bell','bar','suika','cherry','blue7','replay','hero','bell','cherry','red7','suika','replay']
];
...
const state = {
  ...
  enemyA:'highschool_girl', enemyB:'salaryman', pendingBonus:null, setting:1, door:0, doorHits:0,
  presentation:'crate', challenge:null, reelBases:[0,0,0]
};
...
const els = {
 ...
 bonusRate:$('#bonusRate'), bellRate:$('#bellRate'), modeText:$('#modeText'), history:$('#history'), stageBg:$('#stageBg'), hero:$('#hero'), enemyA:$('#enemyA'), enemyB:$('#enemyB'),
...
function renderStageButtons(){
 ...
 document.querySelectorAll('[data-stage]').forEach(btn=>btn.addEventListener('click',()=>setStage(Number(btn.dataset.stage))));
}
function setStage(i){ state.stage=i; els.stageBg.src=backgrounds[i].src; document.querySelectorAll('[data-stage]').forEach(btn=>btn.classList.toggle('active',Number(btn.dataset.stage)===i)); }
function setStageByKey(key){
 const index = backgrounds.findIndex(b => b.key === key);
 setStage(index >= 0 ? index : 0);
}
...
function setHero(action){ const fps = action==='special'?9:action==='shoot'?7:action==='melee'?8:action==='run'?8:3; playFrames(els.hero, heroFrames(action), fps, 'hero', true); }
...
function maxBet(){ if(state.spinning||state.bet===3||state.credit<1||state.challenge)return; const need=3-state.bet; if(state.credit<need)return; state.credit-=need; state.bet=3; state.diff-=need; state.pay=0; update(); }
function pushAction(){
 if(state.challenge && !state.spinning){ advanceChallenge(); return; }
 if(state.pendingBonus && !state.spinning){ startBonus(); return; }
 flash(); els.lcdStatus.classList.remove('empty'); els.lcdStatus.textContent = state.spinning ? 'STOP!' : (state.door>0 ? 'DOOR' : 'PUSH');
 setTimeout(()=>{ if(!state.spinning && !state.pendingBonus && !state.challenge){ els.lcdStatus.classList.remove('empty'); els.lcdStatus.textContent = state.door>0 ? `DOOR ${state.door}` : 'READY'; } }, 300);
}
function leverOn(){
 if(state.spinning||state.bet<3||state.challenge)return;
 clearPrizeScene();
 clearBossBattle();
 const doorActive = state.door > 0;
 els.lcdStatus.textContent=state.pendingBonus ? 'BONUS?' : (doorActive ? 'DOOR' : '...');
 state.games++;
 state.reelBases = reelMap.map(arr => rand(arr.length));
 state.spinning=true; state.stopped=[false,false,false]; state.result=state.pendingBonus ? drawPendingBonusOutcome() : drawOutcome(); state.center=state.result.center; state.presentation=choosePresentation(state.result);
 if(state.door>0) state.door--;
 if(!doorActive && state.games%7===0)setStage(chooseRoamingStage()); if(state.games%4===0)randomizeActors();
 startPrizeScene(state.presentation);
 playEffect(state.presentation === 'freeze' ? 'special' : 'avoid', state.presentation === 'freeze' ? '!!!' : (state.pendingBonus ? 'BONUS?' : (doorActive ? 'DOOR' : '...')));
...
function stopReel(i){ if(!state.spinning||state.stopped[i])return; state.stopped[i]=true; alignReelToIndex(i,state.result?.targetIndices?.[i], true); if(state.stopped.every(Boolean))settle(); }
...
function startBonus(){
 if(!state.pendingBonus||state.spinning)return;
 clearBossBattle();
 clearPrizeScene();
 const type = state.pendingBonus;
 const info = bonusInfo[type];
 state.credit += info.pay;
 state.diff += info.pay;
 state.pay = info.pay;
 ...
 randomizeActors();
 playEffect(type === 'REG' ? 'bonus' : 'door', type !== 'REG' ? `DOOR ${state.door}` : 'REG');
 pushHistory(`${info.label}終了`, info.pay);
 update();
}
...
function update(){
 ...
 els.modeText.textContent=state.challenge?'BONUS CHANCE':state.pendingBonus?`${bonusInfo[state.pendingBonus].label}成立`:state.spinning?'回転中':state.door>0?`DOOR ${state.door}`:'通常';
 ...
 $('#leverBtn').disabled=state.spinning||state.bet<3||!!state.challenge; $('#maxBetBtn').disabled=state.spinning||state.bet===3||state.credit<1||!!state.challenge; if(els.pushBtn) els.pushBtn.disabled=false;
 if(els.settingSelect) els.settingSelect.disabled = state.spinning || !!state.challenge;
}
function reset(){ Object.assign(state,{credit:50,bet:0,pay:0,diff:0,games:0,big:0,reg:0,sbb:0,bell:0,spinning:false,stopped:[true,true,true],result:null,center:['blue7','bell','cherry'],history:[],stage:0,pendingBonus:null,door:0,doorHits:0,presentation:'crate',challenge:null,reelBases:[0,0,0]}); clearPrizeScene(); clearBossBattle(); randomizeActors(); setStage(0); playEffect('miss','READY'); setCenter(state.center); update(); }
